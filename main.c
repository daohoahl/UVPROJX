...
void uart_debug_lcd()
{
	if(uart_flag)
	{
		...
		// --- Dòng 1: Hi?n th? Kİ T? ---
		lcd_put_cur(0, 0);
        // Ki?m tra xem kı t? có in du?c ra màn hình không (ASCII t? 32 d?n 126)
        ...
        {
            // N?u là kı t? di?u khi?n (xu?ng dòng, tab...) thì in d?u ch?m h?i
            sprintf(lcd_buffer, "Char: [?]");
        }
		lcd_send_string(lcd_buffer);
		
		// --- Dòng 2: Hi?n th? Mã HEX và Mã th?p phân ---
        // Ğây là cái quan tr?ng nh?t d? b?t l?i
		lcd_put_cur(1, 0);
